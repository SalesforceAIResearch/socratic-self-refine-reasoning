...
def direct(model: str, question: str):
    if "gpt-5" in model.lower():
        instruction = """You are a precise logical reasoning problem solver. Solve the given logical reasoning problem step by step:
...
def decompose_w_dependencies(model: str, question: str, trajectory: str, answer: str):
    ...
    """
    instruction = """You are tasked with breaking down a logical reasoning problem's reasoning process into a series of sub-questions.
...
def contract(model: str,
             ...
             independent=None,
             dependent=None) -> str:
    instruction = """You are a logical reasoning problem solver specializing in optimizing step-by-step reasoning processes. Your task is to optimize the existing reasoning trajectory into a more efficient, single self-contained question.
...
3. **accuracy**: The optimized question must contain the original question's instructions, including the question's target answer (e.g., the name, color, or shape of the object), and the answer format.
...
def ensemble(model: str, question: str, solutions: list):
    instruction = """You are a precise logical reasoning problem solver. Compare then synthesize the best answer from multiple solutions to solve the following question.
...
""".strip()
# PROMPT_VERIFY_SYSTEM = """
# Please act as an impartial judge and evaluate the correctness of the response provided by an AI assistant to the user question. You will only be given the assistant's answer. Identify any mistakes.
# Be as objective as possible. After providing your analysis, you must rule whether the response is correct or incorrect.
# Output your final verdict by strictly following this format: [[Correct]] if the answer is correct and [[Incorrect]] is the answer is incorrect.
# """.strip()
...
def refine(model: str, question: str, original_cot_response: str, judge_response: str):
    instruction = """You are a precise logical reasoning problem solver. Refine the provided solution to the given logical reasoning problem, step-by-step, by meticulously addressing the judge's feedback.
...
You can freely reason in your response, but please enclose the final answer within <answer></answer> tags.
"""
...
def debate_refine(model: str, question: str, original_cot_response: str, other_agents_responses: list):
    instruction = """You are a precise logical reasoning problem solver. Using the solutions from other agents as additional information, refine your original solution to the given logical reasoning problem, step-by-step.
...
You can freely reason in your response, but please enclose the final answer within <answer></answer> tags.""".format(question=question)
...
def decompose(model: str, question: str, cot_response: str):
    ...
    instruction = """You are tasked with breaking down a logical reasoning problem's reasoning process into a series of **atomic** sub-questions.
...
def check_equivalence(model: str, prediction: str, reference_answers: list):
    instruction = """You are a logical reasoning expert. Given the a logical reasoning expression as the prediction and a list of reference answers, determine the confidence of the prediction.
The prediction is:
{prediction}
...
def plan_summarize(model: str, question: str, cot_response: str):
    return """You are a precise and knowledgeable logical reasoning problem solver. Your task is to: 

1. Summarize high-level planning of the reasoning process of the following solution to the given logical reasoning problem. NOTE: the summary of the planning should be concise, clear, and **not** including any specific calculations. 
...
def refine_plan(model: str, question: str, plan: str, evaluation: str):
    return """You are a precise and knowledgeable logical reasoning problem solver. Refine the following high-level  plan to the given logical reasoning problem based on the evaluation of the planning quality.
...
def direct_w_plan(model: str, question: str, plan: str):
    if "gpt-5" in model.lower():
        instruction = """You are a precise logical reasoning problem solver. Solve the given logical reasoning problem step by step based on the following plan:
